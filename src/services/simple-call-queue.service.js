...
class SimpleCallQueueService {
    ...
    async startCallingForCall(call) {
        try {
            console.log(`🚀 Starting call process for call ID: ${call._id}`);
            ...
            if (agents.length === 0) {
                await this.updateCallStatus(call._id, "missed by agent(s)", "No available agents");
            ...
            for (const agent of agents) {
                ...
                if (result.success) {
                    console.log(`✅ Call successful with agent: ${agent.full_name}`);
                    return { success: true, agent: agent.full_name, call_id: call._id };
                }
                
                console.log(`❌ Agent ${agent.full_name} failed: ${result.reason}`);
            }

            // Step 4: All agents failed
            await this.updateCallStatus(call._id, "missed by agent(s)", "All agents failed to answer");
            return { success: false, reason: "All agents failed" };

        } catch (error) {
            console.error("❌ Call process failed:", error);
            await this.updateCallStatus(call._id, "missed", `Error: ${error.message}`);
            throw error;
        }
    }
    ...
    async tryAgentCall(call, agent) {
        try {
            // Update call status
            await this.updateCallStatus(call._id, "in-progress", `Calling agent: ${agent.full_name}`);
            
            // Add agent to call record
            await this.addAgentToCall(call._id, agent._id, "in-progress");

            // Check if FreeSWITCH is available
            ...
            if (!agentAnswered) {
                ...
                await this.updateAgentStatus(call._id, agent._id, "missed");
                return { success: false, reason: "Agent did not answer" };
            }

            console.log(`✅ Agent ${agent.full_name} answered! Calling lead...`);
            await this.updateAgentStatus(call._id, agent._id, "answered");

            // Step 2: Call lead and bridge
            const leadNumber = call.lead_data.phone_number;
            console.log(`📞 Calling lead: ${leadNumber}`);
            ...
            await this.updateCallStatus(call._id, "answered", "Call connected successfully");
            ...
            return { success: true, agent_uuid: agentUuid, lead_uuid: leadUuid };
     ...
     * Handle call hangup
     */
    async handleCallHangup(callId, uuid, cause) {
        const callInfo = this.activeCalls.get(callId);
        if (!callInfo) return;

        console.log(`📴 Call hangup: ${cause}`);

        // Mark call as completed
        await this.updateCallStatus(callId, "answered", `Call completed - ${cause}`);
     ...
     */
    async updateCallStatus(callId, status, description) {
        try {
     ...
     * Add agent to call record
     */
    async addAgentToCall(callId, agentId, status) {
        try {
            const agentData = {
                id: agentId,
     ...
     */
    async updateAgentStatus(callId, agentId, status) {
        try {
            await this.callRepo.updateByIdAndAccount(callId, {
                $set: { 
                    "agents.$.last_call_status": status,
                    "ringing_agent.last_call_status": status
                },
                updated_at: new Date()
            });
        } catch (error) {
...
module.exports = SimpleCallQueueService;
